...
class AddressBook(UserDict):
    def add_record(self):
        ...
        if existing_records:
            print(
                f"Error: Record: {name} already exists in the address book. Please choose a different name.\n"
            )
            ...
            email = input(
                "If you don't want to add email address just press enter. \nEnter email (example@dummy.com):"
            )
            email_str = ""
            if email.strip():
                try:
    ...
    def add_email(self):
        while True:
            ...
            if not existing_records:
                print(
                    f"Error: This name: {name} does not exist in the address book. Please add a new record first.\n"
                )
                continue
            else:
    ...
    def edit_birthday(self):
        while True:
            ...
            if record.birthday != None:
                ...
                birthday = input(
                    "Press enter if you do not want to change birth date\nEnter new date of birth (dd-mm-yyyy): "
                )
                if not birthday:
    ...
    def find_partial_records(self):
        while True:
            search_char = (
                input("Enter the character by which you want to search for users: ")
                .strip()
                .lower()
            )
            if not search_char:
            ...
            for record in self.data.values():
                if keyword_lower in record.name.value.lower() or any(
                    keyword_lower in str(phone).lower() for phone in record.phones
                ):
                    results.append(record)
            if results:
    ...
    def upcoming_birthdays(self):
        while True:
            try:
                days = int(
                    input("Enter the number of days to search for upcoming birthdays: ")
                )
                if not days:
                ...
                if days < 0:
                    ...
                    pass
                if days == 0:
                    upcoming_bdays_today = [
                        record
                        for record in upcoming_bdays
                        if datetime(
                            today.year,
                            record.birthday.value.month,
                            record.birthday.value.day,
                    ...
                    upcoming_bdays_future = [
                        record
                        for record in upcoming_bdays
                        if (record.birthday.value.month > today.month)
                        or (
                            record.birthday.value.month == today.month
                            and record.birthday.value.day >= today.day
                        )
                    ]
                    if upcoming_bdays_future:
                        print(
                            f"Success: Found contacts with upcoming birthdays within {days} days:"
                        )
                ...
                print(
                    "Error: Please enter a valid number of days for upcoming birthdays search.\n"
                )
